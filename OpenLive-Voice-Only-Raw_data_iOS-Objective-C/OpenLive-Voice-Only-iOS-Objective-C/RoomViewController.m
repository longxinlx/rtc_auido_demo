...
- (void )mediaDataPlugin:(AgoraMediaDataPlugin *)mediaDataPlugin didMixedAudioRawData:(AgoraAudioRawData *)audioRawData
{
    ...
    long success = 0;
       if (self.needWritePCM && currentRawDataDumpType == AgoraSDKRawDataType_OnMixed) {
...
{
    long success = 0;
    if (self.needWritePCM && currentRawDataDumpType == AgoraSDKRawDataType_OnRecord) {
        size_t writeBytes = audioRawData.samplesPerChannel * audioRawData.channels * sizeof(int16_t);
...
{
        long success = 0;
       if (self.needWritePCM && currentRawDataDumpType == AgoraSDKRawDataType_PlayBackBeforeMixing) {
           size_t writeBytes = audioRawData.samplesPerChannel * audioRawData.channels * sizeof(int16_t);
...
{
        long success = 0;
       if (self.needWritePCM && currentRawDataDumpType == AgoraSDKRawDataType_PlayBack) {
           size_t writeBytes = audioRawData.samplesPerChannel * audioRawData.channels * sizeof(int16_t);
...
-(void)startRecordPCM:(AgoraSDKRawDataType)agoraSDKRawDataType
{
    ...
    currentRawDataDumpType = agoraSDKRawDataType;
...
- (IBAction)dumpOnRecordPcm:(UIButton *)sender {
    self.isDumpRecordPcm = !self.isDumpRecordPcm;
       if(self.isDumpRecordPcm) {
           [self.agoraKit setRecordingAudioFrameParametersWithSampleRate:48000 channel:2 mode:0 samplesPerCall:960];
...
- (IBAction)dumpPlayBackPcm:(UIButton *)sender {
    self.isDumpPlayBackPcm = !self.isDumpPlayBackPcm;
          if(self.isDumpPlayBackPcm) {
              [self.agoraKit setPlaybackAudioFrameParametersWithSampleRate:48000 channel:2 mode:0 samplesPerCall:960];
...
- (IBAction)dumpOnMixedPcm:(UIButton *)sender {
    self.isDumpOnMixedPcm = !self.isDumpOnMixedPcm;
          if(self.isDumpOnMixedPcm) {
              [self.agoraKit setMixedAudioFrameParametersWithSampleRate:48000 samplesPerCall:960];
